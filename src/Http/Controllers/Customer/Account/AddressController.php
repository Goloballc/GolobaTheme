...
namespace Webkul\GolobaTheme\Http\Controllers\Customer\Account;
...
class AddressController extends Controller
{
    ...
    public function store(AddressRequest $request)
    {
        ...
        $customerAddress = $this->customerAddressRepository->create($data);
          ...
          // Guardar código DANE si está presente
          if ($request->filled('dane_code')) {
              \DB::table('address_metadata')->insert([
                  'address_id' => $customerAddress->id,
                  'dane_code'  => $request->input('dane_code'),
                  'created_at' => now(),
                  'updated_at' => now(),
              ]);
          }
